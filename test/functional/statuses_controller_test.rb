...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should create a status with only status" do
    ...
    assert_equal body["app_status"]["status"], "DOWN"
  end
  ...
  test "should create a status with only status_message" do
    ...
    assert_response :success
    body = JSON.parse(response.body)
    assert_equal body["app_status"]["status_message"], "Chaos monkey on the loose"
  end
  ...
  test "should create a status with status and status_message" do
    ...
    assert_response :success
    body = JSON.parse(response.body)
    assert_equal body["app_status"]["status"], "UP"
    assert_equal body["app_status"]["status_message"], "All systems go"
  end
